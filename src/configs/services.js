...
export const login = async (dispatch, body, seterrLogin) => {
  try {
    ...
    localStorage.setItem("id", JSON.stringify(getUser.data.data.user.id));
...
export const loadedService = async (dispatch) => {
  try {
    ...
    dispatch({
      type: "LOADED",
      payload: { ...getProfile.data.data, ...getProfile.data.data.user.id },
    });
...
export const getPosts = async (cbSuccess) => {
  try {
    ...
    // return posts.data.data.posts.map((post) => {
    //   return {
...
export const addArt = (data, cbSuccess) => {
  const config = {
    headers: {
      "content-type": "multipart/form-data",
    },
